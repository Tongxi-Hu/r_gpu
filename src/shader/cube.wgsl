...
fn vs_main(in: Input) -> Inter {
    ...
    inter.surface_vector = vec4<f32>(in.norm, 1.0) * (rotation_x * rotation_y * rotation_z);
...
fn rotation_3d_x(rotation_deg: f32) -> mat4x4<f32> {
    return mat4x4<f32>(vec4<f32>(1.0, 0.0, 0.0, 0.0), vec4<f32>(0.0, cos(PI * rotation_deg / 180.0), - sin(PI * rotation_deg / 180.0), 0.0), vec4<f32>(0.0, sin(PI * rotation_deg / 180.0), cos(PI * rotation_deg / 180.0), 0.0), vec4<f32>(0.0, 0.0, 0.0, 1.0));
}

// generate 3d rotation matrix around y axis
fn rotation_3d_y(rotation_deg: f32) -> mat4x4<f32> {
    return mat4x4<f32>(vec4<f32>(cos(PI * rotation_deg / 180.0), 0.0, sin(PI * rotation_deg / 180.0), 0.0), vec4<f32>(0.0, 1.0, 0.0, 0.0), vec4<f32>(- sin(PI * rotation_deg / 180.0), 0.0, cos(PI * rotation_deg / 180.0), 0.0), vec4<f32>(0.0, 0.0, 0.0, 1.0));
}

// generate 3d rotation matrix around z axis
fn rotation_3d_z(rotation_deg: f32) -> mat4x4<f32> {
    return mat4x4<f32>(vec4<f32>(cos(PI * rotation_deg / 180.0), - sin(PI * rotation_deg / 180.0), 0.0, 0.0), vec4<f32>(sin(PI * rotation_deg / 180.0), cos(PI * rotation_deg / 180.0), 0.0, 0.0), vec4<f32>(0.0, 0.0, 1.0, 0.0), vec4<f32>(0.0, 0.0, 0.0, 1.0));
}

// generate 3d translation matrix
fn translation_3d(translation: vec4<f32>) -> mat4x4<f32> {
    return mat4x4<f32>(vec4<f32>(1.0, 0.0, 0.0, translation.x), vec4<f32>(0.0, 1.0, 0.0, translation.y), vec4<f32>(0.0, 0.0, 1.0, translation.z), vec4<f32>(0.0, 0.0, 0.0, 1.0));
}
